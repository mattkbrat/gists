...
const getIdiomatics = () => {
    ...
    return idiomatics.filter((i) => i instanceof HTMLElement && i.classList.contains("char"));
};
const isValidCode = (element) => {
    // return element.dataset.class?.startsWith("23");
    return element.tagName === "CODE" && element.dataset.class?.startsWith("23");
};
const isValidDiv = (element) => {
 ...
 */
const isValidSpan = (element) => {
    return element.tagName === "SPAN" && element.dataset.id?.includes("21");
};
/**
 * Decodes the current document to match criteria specified in ctf.md
 */
const getText = (idiomatics) => {
    let text = "";
